...
const sql = fs.readFileSync(
  path.join(
    "supabase",
    "migrations",
    "20260805180000_segments_origin.sql",
  ),
  "utf8",
);
...
try {
  ...
  const cols = await client.query(`
    ...
    where table_schema='public' and table_name='segments'
      and column_name in ('origin','source_key')
    order by 1
  `);
  const origins = await client.query(`
    select origin, count(*)::int as n from segments group by origin order by 1
  `);
  console.log("OK_COLS=" + cols.rows.map((r) => r.column_name).join(","));
  console.log(
    "OK_ORIGINS=" +
      origins.rows.map((r) => `${r.origin}:${r.n}`).join(","),
  );
} catch (error) {
  console.error(
